...
**冪等性の注意(P3ゲート③検証レビューverify-C5対応):** `R__`はチェックサム変更のたびに再実行されるため、単純な`INSERT`だと重複行が入りうる。`sentences`はUNIQUE制約が無く特に事故りやすいため、`ON CONFLICT`(`topic_sets`)または全削除→再投入(`sentences`)で冪等に書く。実ファイル(`R__seed_topic_master.sql`)はP5-05で作成済み、`docker compose`での複数回起動・`R__`再実行で重複しないことを実機確認済み。
