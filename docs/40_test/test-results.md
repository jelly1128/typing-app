...
## P6-05: ST-014〜015(NFR-01/02計測、2026-09-12)

`frontend/e2e/nfr-performance.spec.ts`に実装。test-plan.md 2.3節・CL-027どおり、Playwrightテストコード内で計測する(NFR-01はkeydown発火時刻〜2フレーム後のブラウザ内計測、NFR-02はNode側`Date.now()`でクリック〜描画完了までを計測)。個人学習用途のため厳密な性能試験ではなく「目安確認」(閾値は目標値よりゆるく設定し、異常な遅延の有無だけを見る)。

| ID | 概要 | 実測値 | 結果 |
|---|---|---|---|
| ST-014 | NFR-01入力反応性(目標50ms以内) | 平均29.6ms、最大36.3ms(3サンプル) | PASS(目標値以内) |
| ST-015 | NFR-02画面表示速度(目標2秒以内) | S-02→S-03=88ms、S-04→S-05=52ms | PASS(目標値以内) |
