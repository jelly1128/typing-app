...
| **次にやること** | P5-16〜17(views続き: ResultView/SessionMetricsSummary・HistoryView/MissAnalysisView)から着手 |
| **開始日** | 2026-08-22 |
...
| 2026-09-08〜09 | P5 | `/ty-plan`で前回の打ち手(「複雑に見えるか」ではなく「設計書に未解決の判断が残っているか」を確認)を実際に適用: 着手前に`screen-design.md`(S-03)/`class-design.md`(2.2/2.8/2.9)を読み、P5-15をブロックする未解決の設計判断が無いことを確認してから見積もりに入った。Kazuki1.0h/Claude1.5h(差分1.5倍、キーボードイベント処理・タイマー表示・自動遷移の配線分)で突き合わせ、Kazukiも同意し合意値1.5hで着手。P5-15(`TypingView.vue`/`TypingDisplay.vue`、S-03)を実装: `window`のkeydownを`sequenceJudge`に直接渡し`sessionStore.recordKeystroke`で集計、お題文は配列を先頭から使い末尾で戻るローテーション(doc-B8)、sentence_count/time_limitそれぞれの終了条件判定(time_limitは200ms間隔タイマーで残り秒数表示と判定を兼ねる)、終了時`sessionStore.endSession()`→`finished`をemit(ルーティングはP5-18へ委譲、P5-14と同方針)。設計書に無い実装判断3点(判定エンジンの状態保持場所・文完了検知方法・time_limit終了判定のタイマー化)をKazukiに説明し「とくにない」で了承を得た。コンポーネントテスト9件(`TypingView`6件+`TypingDisplay`3件)全Green、`vue-tsc -b`型チェック通過。実績1.5h(合意値どおり、ズレ率0%)。作業中、git stop-hookがタスク境界(wbs.md更新後・`npm install`によるpackage-lock.jsonのメタデータ差分発生後)で都度コミットを要求してきたため、通常の`/ty-end`を待たずセッション中に複数回コミット・pushした(内容を都度確認し、無害なnpm メタデータ差分と実質的な変更を分けて別コミットにした)。**P5-15完了** | P5-16〜17(views続き)から |
...
~~**(2026-09-08確定)Claudeドラフトタスクの複雑度評価:** P5-13(`sessionStore`)で、状態を持つロジックという「複雑度」を理由にP5-04相当と見積もり合意値を1.5hに引き上げたが、実績1.0h(-33%)。要因は設計書(`romaji-automaton.md`7.1)に生成規則が既に確定済みで、実装時に解くべき曖昧さがほぼ無かったこと。次回以降、Claudeドラフトタスクを「複雑そうに見える」という理由で高く見積もる前に、「対応する設計書(logic-spec/class-design.md等)に未解決の判断が残っているか」を先に確認する。残っていなければ複雑度に関わらず標準倍率(0.3〜0.5倍)を適用してよい。~~ **→ 実行・再現確認済み(2026-09-08)。** P5-15の`/ty-plan`冒頭で実際にこの確認を行い(`screen-design.md`/`class-design.md`を読み未解決判断が無いことを確認)、Kazuki1.0h/Claude1.5hの差分1.5倍の根拠(未解決判断ではなくキーボード/タイマー配線の分)を明示した上で合意、実績もズレ率0%で的中した。2回連続(P5-13の事後検証→P5-15の事前適用)で機能が確認できたため、**次回以降はこの確認を残りのP5タスク(P5-16/17/19)にも標準適用し、都度「実行しましたか」と聞き返す必要はない。ただし確認の結果、対応する設計書に実際に未解決の判断が見つかった場合は、その旨を明示した上で見積もりを高めに調整する。**
...
| 2026-09-08〜09 | P5 | P5-15セッション中、git stop-hookが`/ty-end`を待たずセッション途中(wbs.md更新後、`npm install`実行によるpackage-lock.jsonのメタデータ差分発生後)に「未コミットの変更がある」と複数回介入してきた。特にpackage-lock.jsonは実質的な依存関係変更ではなく、環境のnpmバージョンが生成した`libc`フィールドの有無だけの差分だった | このプロジェクトの運用(コミットは`/ty-end`でまとめて、または`次回の打ち手`にある「タスク区切りで都度」)は、hookが持つ「未コミット変更ゼロ」の期待とは独立に動いている。hookが介入した時点で、①差分の中身を確認し、②意図した変更(実装ファイル)と無関係なノイズ(npmメタデータ等)を分けて、③ノイズは無害と判断できれば素直にコミットしてよい(無理に除外・調査に時間をかけない)。次回以降も同様のnpmメタデータ差分が出た場合は再調査不要、同じ扱いでよい |
