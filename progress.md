...
|---|---|
| **現在の工程** | P2 基本設計(P1 完了) |
| **次にやること** | セッション冒頭で doc-reviewer が Opus で起動しているか確認 → 可能なら同じP2文書一式で再レビューしSonnet版(REV-004)と比較 → P2-06(test-reviewer/ops-reviewer作成) |
| **開始日** | 2026-08-22 |
...
- **Claude が全文をドラフトし、Kazuki が意思決定点(選択肢の確認・承認)でのみ介在するタスク(設計文書作成全般)は、見積もりを Kazuki の直感値の3〜4割程度に下方修正して合意する。**(2026-08-23、P2-00〜P2-05の合意値合計8.75h(525分)に対し実績66分、ズレ率約-87%。P1-07の教訓「レビュー委譲タスクは見積もりを下げてよい」の適用範囲を、レビュー委譲だけでなく「Claudeがドラフトし人間は意思決定のみ」というタスク全般に広げる)
