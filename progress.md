...
| **次にやること** | P3のタスク分解(P3-00)から着手。`class-design.md` / `sequence.md` / `logic-spec/romaji-automaton.md` / `db-access.md` を作成しゲート③へ |
| **開始日** | 2026-08-22 |
...
- **「初めての作業」「環境構築」というカテゴリだけで見積もりを高くしない。調査・設定ファイル作成をClaudeが担い、Kazukiの実働がUI操作/確認のみと分かっているタスクは、初回でも低め側の値を採用する。**(2026-08-29、P2.5-06 合意値1.75hに対し実績0.75h、ズレ率-57%。「初回インフラ構築だから」で高く見積もったが、実際はRenderドキュメント調査・Dockerfile作成・トラブルシューティングをClaudeが巻き取り、Kazukiの実働はダッシュボード操作のみだった。P1-07/P2-05/P2-06b/P2-07と同型のズレ)
