...
| 項目 | 内容 |
|---|---|
| **現在の工程** | P3 詳細設計(P2.5 完了) |
| **次にやること** | P3-09(ゲート③レビュー実施と指摘対応)。セッション冒頭で`.claude/rules/`(typing-core.md/judgment-engine.md)が読み込まれることを確認する |
| **開始日** | 2026-08-22 |
...
- **P3-09(ゲート③)の見積もりは、「レビュー起動(エージェントに委譲し待つだけ)」と「指摘対応(実際に複数文書を書き直す)」を分けて見積もる。** レビュー起動分は低い合意値(0.2〜0.3h程度)で先に合意し、指摘対応分は指摘票(重要度A件数)が出てから追加で見積もる2段階方式にする。(2026-08-29。前回の打ち手[誰が実際に手を動かすかを自問する]はP3-04〜08で事前適用に成功しズレ率0%を達成できた[estimate-actual.md参照]。今回はその発展として、2026-08-26のP2-06a/P2-07で得ていた「レビュー系タスクは起動と指摘対応で見積もりの性質が違う」という教訓をP3-09で再適用する)
