...
| 項目 | 内容 |
|---|---|
| **現在の工程** | P4 テスト設計(P3 完了) |
| **次にやること** | P4タスク分解と見積もり。`wbs.md`の「B/C指摘の対応フェーズ振り分け」P4担当分(test-B1/B2/B3, test-C1, test-C3/ops-C3, doc-B9/ops-B8/test-C4)を最初に拾う |
| **開始日** | 2026-08-22 |
...
- [ ] **P5** 実装 — ローカルで MVP の全 FR が動く状態 **【着手時必読】`typing-core`(ロジックコア)はKazuki自身がTDDで書く方針。Claudeはドラフトを書かず、仕様通りかのレビューのみ行う。理由: P3セッション(2026-08-29)で「設計書を読んで判断するだけで実装力が身についているか」という懸念が出たため([wbs.md](./docs/00_project/wbs.md#p5-実装--6〜10-セッション)/[workflow.md](./workflow.md#p5-実装)にも同旨記載)
...
- **A指摘の「指摘対応」見積もりには、自己修正後の検証レビュー(戻りのA指摘対応)分を最初から内訳に含めて見積もる。** 同じ会話のClaudeが書いた修正ドラフトは、自分の設計を肯定しやすいのと同じ理屈で「自分の修正は正しい」とも思い込みやすい。今回はP3-09の指摘対応(合意値1.5h)の中で検証パス(REV-013、A7件新規検出)への対応も収まった(実績0.95h、-37%)が、これは2段階見積もり(起動/指摘対応)の外側で偶然吸収された分であり、次回は指摘対応の内訳自体に「検証→戻り対応」の一往復を明示的に織り込む。(2026-08-30。P3-09で「1人でレビュー・修正・検証まで一気通貫で回せた」ことが下振れに効いたと分析済み[estimate-actual.md参照]だが、検証パスが標準工程として意識されていなければ次回タスクで抜け落ちるリスクがある)
