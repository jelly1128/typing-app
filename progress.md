...
|---|---|
| **現在の工程** | P5 実装(P4 完了) |
| **次にやること** | P5タスク分解と見積もり。着手時に相談すること: (1) `typing-core`以外のレイヤー(API/フロント/hooks)の分担、(2) `shared/testdata`はKazukiがTDDで書きながら埋める方針(2026-08-30合意、`test-plan.md`3章末尾)、(3) E2Eフレームワーク(Playwright等)・Testcontainers導入の選定 |
| **開始日** | 2026-08-22 |
...
- [ ] **P5** 実装 — ローカルで MVP の全 FR が動く状態 **【着手時必読】`typing-core`(ロジックコア)はKazuki自身がTDDで書く方針。Claudeはドラフトを書かず、仕様通りかのレビューのみ行う。理由: P3セッション(2026-08-29)で「設計書を読んで判断するだけで実装力が身についているか」という懸念が出たため([wbs.md](./docs/00_project/wbs.md#p5-実装--6〜10-セッション)/[workflow.md](./workflow.md#p5-実装)にも同旨記載)。あわせて`shared/testdata`(P4で形式のみ決定、本体は未作成)もP5でKazukiがTDDのテストを書きながら埋める方針(2026-08-30、[test-plan.md](./docs/40_test/test-plan.md)3章末尾)**
